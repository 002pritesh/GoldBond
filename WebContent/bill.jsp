...
try{
	...
	ResultSet rs2=st.executeQuery("select *from users inner join cart where cart.email='"+email+"'amd cart.status='bill'");
	while(rs.next()){
		

%>
<h3>Online Gold Bond  Bill (GGB) </h3>
<hr>
...
<h3>Product Details</h3>
  ...
  <%
  ResultSet rs1=st.executeQuery("select *from cart join product where cart.productId=product.id and cart.email'"+email+"' and cart.status='bill'"); 
  while(rs1.next()){
...
<a href="continueBuying.jsp"><button class="button left-button">Continue Shopping</button></a>
